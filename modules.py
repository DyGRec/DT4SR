...
def multihead_attention(queries, keys, values, key_masks,
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        # Linear projections
        Q = tf.layers.dense(queries, d_model, use_bias=True, activation=tf.nn.elu) # (N, T_q, d_model)
        K = tf.layers.dense(keys, d_model, use_bias=True, activation=tf.nn.elu)  # (N, T_k, d_model)
        V = tf.layers.dense(values, d_model, use_bias=True, activation=tf.nn.elu) # (N, T_k, d_model)
...
def feedforward(inputs, num_units, scope="positionwise_feedforward"):
    ...
    with tf.variable_scope(scope, reuse=tf.AUTO_REUSE):
        ...
        # Outer layer
        outputs = tf.layers.dense(outputs, num_units[1], activation=tf.nn.elu)
